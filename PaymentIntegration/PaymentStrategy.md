...
| :--------------- | :------ | :--------------------------- | :------------------------------------------------- |
| **Razorpay**     | Gateway | 🇮🇳 India Focus, Startups     | International failure rates can be higher.         |
...
_Mental Model: Create Order on Server → Open Modal on Client → Verify Signature on Server._
...
function payWithRazorpay() {
  const options = {
    ...
    name: 'My Store',
    order_id: backendConfig.rzp_order_id, // ⚠️ CRITICAL: Must come from backend
    ...
    handler: function (response) {
      ...
      // 🚀 Step 3: Send this data to YOUR backend to verify signature
...
_Mental Model: Create Intent → Confirm Intent. Stripe handles 3D Secure automatically._
...
async function payWithStripe() {
  ...
  if (error) {
    ...
    // 🚀 Inform backend (Optional, strictly rely on Webhook)
  }
}
```
  ...
  const options = {
    ...
    callback: (result) => {
      if (result.status === 'CHARGED') {
        ...
        // 🚀 CRITICAL: Verify this on your backend using Juspay's server-to-server API
      } else {
...
## 6. Pitfalls & "Gotchas" ⚠️

1.  **Trusting the Frontend:**
    - ❌ _Bad:_ Frontend sends `{ status: "success", amount: 100 }` to backend.
    - ✅ _Good:_ Frontend sends `{ payment_id: "pay_123" }`. Backend asks Gateway: "Did pay_123 actually pay us 100?" **Always verify payment status and amount on the backend.**

2.  **Currency Decimals (Lowest Unit):**
    - Stripe/Razorpay usually expect amounts in **cents/paise** (Integers).
    - $10.00 = `1000` (cents).
    - ❌ Sending `10` might charge 10 cents. **Always convert to the lowest currency unit (e.g., multiply by 100 for USD/INR) before sending to the gateway.**
...
1.  **Frontend:** UI → Collects User Intent → Initiates Payment (opens Gateway Modal/Redirects).
2.  **Backend:** Generates Secure Transaction (Order ID/Client Secret) → Verifies Payment Status (Server-to-Server) → Listens to Webhooks (CRITICAL for reliability).
