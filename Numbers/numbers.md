...
_Covers the built-in ways JavaScript handles numbers._
...
**Examples:** `1e3` (1 _ 10^3 = 1000), `5e-4` (5 _ 10^-4 = 0.0005)
...
_Fundamental categories of numbers from classical mathematics._
...
_Numbers defined by properties of their own digits._
...
_Numbers related to divisors, combinations, and famous mathematical sequences._

### 4.1 Divisor-Related Numbers

#### 4.1.1 Perfect, Abundant, and Deficient Numbers

**Definition:** Based on the sum of a number's _proper divisors_ (divisors excluding the number itself).
...
**Definition:** Sequence where each number is the sum of the _three_ preceding ones. Starts with `0, 1, 1`.
...
```js
function catalan(n) {
  // Iterative formula, avoids large factorials
  let c = 1;
  for (let i = 0; i < n; i++) {
    c = (c * 2 * (2 * i + 1)) / (i + 2);
  }
