...
*   **Stack**: When a `Date` variable is declared (e.g., `now` in `const now = new Date()`), the variable itself is stored on the **call stack**. This variable holds the *memory address* (reference) of where the actual `Date` object data is located.
...
*   Methods like `getHours()`, `getDate()` return values based on the *local* time zone.
*   Methods like `getUTCHours()`, `getUTCDate()` return values based on *UTC*.
*   When creating `new Date(year, month, ...)` it creates a date in *local* time.
