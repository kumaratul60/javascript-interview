...
In non-strict mode, you can accidentally create global variables if you don't declare them with `var`, `let`, or `const`. If you try to access an undeclared variable, it will be `undefined` *after* the initial attempt, but `typeof` will return `"undefined"` before an error is thrown.
