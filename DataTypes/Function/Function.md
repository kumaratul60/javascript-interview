...
- **Stack**: When a function variable is declared (e.g., `greet` or `sayHi`), the variable itself is stored on the **call stack**. This variable holds the _memory address_ (reference) of where the actual function object is located.
...
The value of `this` inside a function depends entirely on _how_ the function is called. This is a common source of bugs and a frequent interview topic.
...
A classic closure pitfall. If a closure captures a loop variable, it captures the _final_ value of that variable after the loop finishes.
