...
- **Stack**: When a `Promise` variable is declared (e.g., `myPromise`), the variable itself is stored on the **call stack**. This variable holds the _memory address_ (reference) of where the actual `Promise` object data is located.
...
3.  **Parallel Execution of Asynchronous Tasks**: `Promise.all()`, `Promise.race()`, `Promise.allSettled()`, `Promise.any()` enable powerful parallel and competitive execution patterns.
    ...
    Promise.all([fetch('/api/user/profile'), fetch('/api/user/settings')])
...
Callbacks passed to `.then()`/`.catch()` are executed in the **microtask queue**, which has higher priority than the **macrotask queue** (where `setTimeout`, `setInterval` callbacks run). This means microtasks execute _before_ macrotasks.
...
The executor function (`(resolve, reject) => { ... }`) passed to `new Promise()` is executed _synchronously_. This can lead to subtle bugs if you expect it to be entirely asynchronous.
